...
export async function POST(request: Request) {
  try {
    ...
    // Validate required fields including classLevel
    if (!timetableData.className || !timetableData.classLevel || 
        !timetableData.department || !timetableData.days) {
    ...
    // Validate classLevel format
    if (!/^Class [1-9]|10$/.test(timetableData.classLevel)) {
      return NextResponse.json(
