...
interface ClassData {
  className: string;
  classLevel: string;  // Ensure this is included
  stream: string;
  courses: string[];
  teachers?: { 
    course: string; 
...
const [submissionStatus, setSubmissionStatus] = useState<{
  ...
  const handleSaveTimetable = async () => {
    try {
      // Find the selected class to get its classLevel
      const selectedClassData = classes.find(c => c.className === selectedClass);
      if (!selectedClassData) throw new Error("Class data not found");
  
      const timetableData = {
        className: selectedClass,
        classLevel: selectedClassData.classLevel, // Add class level
